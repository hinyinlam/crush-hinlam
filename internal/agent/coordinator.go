...
func (c *coordinator) buildTools(ctx context.Context, agent config.Agent, isSubAgent bool) ([]fantasy.AgentTool, error) {
	...
	allTools = append(
		...
		tools.NewBashTool(c.permissions, c.cfg.WorkingDir(), c.cfg.Config().Options.Attribution, modelID,
			time.Duration(c.cfg.Config().Options.ToolTimeoutSeconds)*time.Second,
			time.Duration(c.cfg.Config().Options.BackgroundJobTimeoutSeconds)*time.Second,
		),
...
func (c *coordinator) buildAnthropicProvider(baseURL, apiKey string, headers map[string]string, providerID string) (fantasy.Provider, error) {
	...
	httpClient := log.NewRetryHTTPClientWithDebug(c.cfg.Config().Options.Debug)
